...
@Composable
fun NewbieScreen(isInsert: MutableState<Boolean>,
                   childName:String,
                   viewModel: NewbieViewModel = hiltViewModel()) {
    ...
    if (isUpdate.value){
        updateNewbie(isUpdate = isUpdate, itemState = viewModel.updateRes.value , viewModel = viewModel,childName)
    }
    ...
    if(res.item.isNotEmpty()){
        LazyColumn{
            items(res.item, key = {
                ...
                EachRowNewbie(itemState = res.item!!, onUpdate = {
                    isUpdate.value=true
...
@Composable
fun EachRowNewbie( itemState: RealTimeModelResponse.RealTimeItems,
             onUpdate:()->Unit,
             onDelete:()->Unit,){
